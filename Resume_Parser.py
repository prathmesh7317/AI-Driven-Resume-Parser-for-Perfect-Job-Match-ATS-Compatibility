...
def get_gemini_response(input_text, doc_content, prompt):
    try:
        ...
        response = model.generate_content([input_text, doc_content, prompt])
            ...
            if st.button("SEND", key="sending_button_4"):
                with st.spinner("Processing your resume..."):
                    ...
                    if "preprocess_doc_content_4" in data and data["preprocess_doc_content_4"]:
                        ...
                        # Call Gemini API
                        response = get_gemini_response(data["job_description_preprocess"], data["doc_content"], input_prompt4)
                        if response:
            ...
            if preprocess_doc_content:
                if input:
                    if submit1:
                        ...
                        response = get_gemini_response(st.session_state.preprocess_JD, doc_content, input_prompt1)
                        st.write(response)
                    if submit3:
                        response = get_gemini_response(st.session_state.preprocess_JD, doc_content, input_prompt3)
                        st.write(response)
                else:
                    st.error('Please update Job Description')
