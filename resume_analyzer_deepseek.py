...
class ATS_System:
    def __init__(self):
        ...
        if not self.api_key:
            st.error("API key not loaded. Check your .env file for the DEEPSEEK_API_KEY variable.")
        else:
    ...
    def extract_jd_info(self, job_description):
        input_prompt_jd_extraction = """
            ...
            Job Description:
        """
        response = self.get_deepseek_response(None,job_description, input_prompt_jd_extraction)
        if response:
            try:
    ...
    def get_deepseek_response(self, input_text, doc_content, prompt):
        try:
            if not input_text:
    ...
    def ask_deepseek(self, question, resume_text):
         prompt = f"""
                    ...
                    if preprocess_doc_content:
                        if input_jd:
                            if submit1:
                                ...
                                response = self.get_deepseek_response(st.session_state.preprocess_JD, doc_content, self.input_prompt1)
                                st.write(response)
                            if submit3:
                                response = self.get_deepseek_response(st.session_state.preprocess_JD, doc_content, self.input_prompt3)
                                st.write(response)
                        else:
                            st.error('Please Mention Job Description')
            ...
            if submit6:
                ...
                if st.session_state.get('doc_content', None):
                   ...
                   if user_query and send_button:
                        response = self.ask_deepseek(user_query, st.session_state.doc_content)
                        st.markdown("""
                <style>
                .response-container {
